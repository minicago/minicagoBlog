...
>数学上**变换**其实是指一类特殊的映射，通常是将函数（可离散）映射成另一函数
...
> 此处的正交性指：两个信号a,b正交当且仅当其卷积为$\mathbf{0}$
> $$(a * b)(x) = 0$$
...
此时 $$b_i = e^{-i\frac{2\pi\cdot i\cdot0}{N}} \, x[0] + e^{-i\frac{2\pi\cdot i\cdot1}{N}} \, x[1] + \cdots + e^{-i\frac{2\pi\cdot i\cdot(N-1)}{N}} \, x[N]$$

即 $b_i$ 为 $f(e^{-i\frac{2\pi\cdot i}{N}})$

其中函数 $$f(a) = \sum_{i=0}^N x[i] \cdot a^i $$

>最终分解的任务就变成求解函数在单位圆上各个单位根的取值，数学真是奇妙啊
>换个角度看待这个问题我们在**傅里叶变换**（上述操作）中所做的，其实是把一个多项式函数中$N$个点的值计算出来了，此时卷积即多项式乘法，所以之后可以直接用乘法O(N)计算出相乘的值，这就是常说的插值表示法转为点值表示法
>点值表示法的和正交分解殊途同归，这正是数学的美妙之处，如果只想到点值表示法，永远也无法理解为什么会选择那几个特殊的值，才能让计算快速。
